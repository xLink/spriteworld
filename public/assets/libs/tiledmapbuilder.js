...
Crafty.c("TiledMapBuilder", {
	...
	isValid: function( source ){
        ...
        if (!source) {
            isValid = false;
            console.log('source not object');
        }

        // has width and height property
        if (!(source.width && source.height)) {
            isValid = false;
            console.log('width or height missing');
        }

        // has no empty layer property
        if (!(source.layers && source.layers.length >=1)) {
            isValid = false;
            console.log('no layers found');
        }

        // has no empty tilesets property
        if (!(source.tilesets && source.tilesets.length >=1)) {
            isValid = false;
